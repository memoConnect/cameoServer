...
//    }
//
//    "add new identity to account without reservation secret" in {
//      val path = basePath + "/identity"
//      val json = Json.obj("displayName" -> displayName, "phoneNumber" -> tel, "email" -> email, "cameoId" -> cameoId)
//
...
//      (data \ "userKey").asOpt[String] must beSome
//      (data \ "cameoId").asOpt[Seq[JsObject]] must beSome
//      (data \ "email" ).asOpt[String] must beNone
//      (data \ "phoneNumber").asOpt[String] must beNone
//
//    }
  }
}